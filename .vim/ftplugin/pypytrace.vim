...
"Edd
function! PyPyTraceFold(lnum)
	if getline(a:lnum-1) =~? '^# \(Loop\|bridge\)' || getline(a:lnum-1) =~? '{jit-backend-dump$'
		return 'a1'
	endif

	if getline(a:lnum) =~? '--end of the loop--$' || getline(a:lnum) =~? 'jit-backend-dump}$'
		return 's1'
	endif
...
set foldmethod=expr
set foldexpr=PyPyTraceFold(v:lnum)
